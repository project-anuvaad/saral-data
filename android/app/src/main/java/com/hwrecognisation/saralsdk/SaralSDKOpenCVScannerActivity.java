...
import com.hwrecognisation.saralsdk.hwmodel.PredictionListener;
import com.hwrecognisation.saralsdk.opencv.DetectShaded;
import com.hwrecognisation.saralsdk.opencv.TableCornerCirclesDetection;
...
public class SaralSDKOpenCVScannerActivity extends ReactActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    private String mRollNumbersPool                     = null;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if(b != null) {
            mScannerType        = b.getInt("scanner", 1);
            mRollNumbersPool    = b.getString("NUMBER_POOL");
            mlayoutConfigs      = GJScannerAndROIs.loadJSONFromAsset(this, mScannerType);

            Log.d(TAG, "LayoutConfigs: " + mlayoutConfigs);
            Log.d(TAG, "mScannerType: " + mScannerType);
            Log.d(TAG, "mRollNumbersPool: " + mRollNumbersPool);
        }
    ...
    private JSONArray getROIs() {
        try {
            ...
            JSONObject layoutROI        = layoutConfigs.getJSONObject("roi");
    ...
    private void processScanningCompleted() {
        ...
        JSONObject  response = GJScannerAndROIs.formatScanResultAsGJPAT_5QROIs(getScanResult());

        Log.d(TAG, "Scanning completed !!, OMR count: " + mPredictedOMRs.size() + " classifier count: " + mPredictedDigits.size());
        Log.d(TAG, "SDK Response : " + response.toString());
